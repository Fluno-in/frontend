...
import { Clock, DollarSign, CheckCircle, XCircle, AlertCircle, Tag, Calendar, List } from 'lucide-react';
import Card from '../../../components/ui/Card';
import Button from '../../../components/ui/Button';
import { Ad, fetchAvailableAds, applyForAd, markAdNotInterested } from '../../../services/influencerDashboard/availableAds';
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <motion.div
        ...
        exit={{ opacity: 0, y: 20 }}
        className={`rounded-lg p-4 shadow-lg ${type === 'success' ? 'bg-green-100' : 'bg-red-100'}`}
      >
...
const InfluencerAds = () => {
  ...
  useEffect(() => {
    const loadAds = async () => {
      try {
        const data = await fetchAvailableAds();
        if (data.success) {
  ...
  const handleApply = async (adId: string) => {
    try {
      const data = await applyForAd(adId);
      if (data.success) {
  ...
  const handleReject = async (adId: string) => {
    try {
      const data = await markAdNotInterested(adId);
      if (data.success) {
        setAvailableAds((prevAds) => prevAds.filter((ad) => ad._id !== adId));
        showToast('Campaign removed from your feed', 'error');
      } else {
            ...
            <Card className="overflow-hidden shadow-lg rounded-lg">
              <div className="relative h-52">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <h3 className="text-xl font-semibold text-white truncate">{ad.campaignName}</h3>
                </div>
              </div>
